...
function setupFloatingWhatsApp() {
  const language = getCurrentLanguage();
  const bubbleText = {
    tr: "Bize istediğiniz yerin videosunu atın hemen fiyat verelim!",
    en: "Send us a short video of the space and we will quote quickly.",
    ru: "Отправьте короткое видео помещения, и мы быстро рассчитаем цену."
  };
  const ariaText = {
    tr: "WhatsApp üzerinden bizimle iletişime geçin",
    en: "Contact us on WhatsApp",
    ru: "Связаться с нами в WhatsApp"
  };
  const widgetHtml = `
    <div class="whatsapp-widget">
      <div class="whatsapp-bubble" id="waBubble">
        ${bubbleText[language] || bubbleText.tr}
      </div>
      <a href="${WHATSAPP_DIRECT_URL}" target="_blank" class="whatsapp-button" aria-label="${ariaText[language] || ariaText.tr}">
